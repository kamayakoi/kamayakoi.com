...
export async function generateMetadata({ params }: PageProps) {
  ...
  const currentLanguage = getPageLocale(paramsData);
  ...
  try {
    ...
    if (!post) {
      return {
        title: t(currentLanguage, "storyPage.notFoundTitle"),
      };
    }

    const title = currentLanguage === 'fr' && post.title_fr ? post.title_fr : post.title;
    const excerpt = currentLanguage === 'fr' && post.excerpt_fr ? post.excerpt_fr : post.excerpt;

    return {
      title: `${title} | Stories | Kamayakoi`,
      description: excerpt,
    };
  } catch {
    return {
...
async function StoryPage({ params }: PageProps) {
  try {
    ...
    const title = currentLanguage === 'fr' && post.title_fr ? post.title_fr : post.title;
    const excerpt = currentLanguage === 'fr' && post.excerpt_fr ? post.excerpt_fr : post.excerpt;
    const body = currentLanguage === 'fr' && post.body_fr ? post.body_fr : post.body;
              ...
              <article className="max-w-3xl sm:max-w-4xl mx-auto pb-10">
                <header className="mb-6">
                  <div className="flex flex-wrap items-center text-sm text-zinc-600 dark:text-zinc-400 mb-6">
                    {false && post.categories && post.categories.length > 0 && (
